...
def collateData(shape, dict,):
    ...
    for i in seq(0.1,5,0.1):
        ...
        final2 = len(ak.flatten(n, axis=None))
...
def canvas(y):
    ...
    sea.despine()
    plt.yticks(np.arange(0, max(y) +5000, 10000))                                                                                                                                 
...
plt.legend()
plt.title("Neutron Production vs Proton Energy - 1000 Protons")
plt.xlabel("Energy of Protons (GeV)")
plt.ylabel("N. of neutrons")
plt.show()
