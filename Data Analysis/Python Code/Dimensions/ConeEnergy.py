...
a, b = np.polynomial.polynomial.polyfit(list(conedict.keys()), list(conedict.values()), 1)
#add points to plot
plt.scatter(list(conedict.keys()), conedict.values())
#add line of best fit to plot
plt.plot(list(conedict.keys()), b*np.array(list(conedict.keys())) + a)

plt.legend(loc = "upper left", prop={'size': 5})
plt.title("Neutron Energy vs Heigh of Cone")
plt.xlabel("Height of Cone (cm)")
plt.ylabel("Kinetic Energy of Neutrons (KeV)")
import tikzplotlib as tk
tk.save("ConeEnergy.tex")
plt.show()
